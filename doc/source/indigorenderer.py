...
def switchIndigoLoader(indigo, options):
    ...
    if not loader:
        raise IndigoRendererError('Cannot find indigo loader for object type: %s, %s' %
                                  (indigo_object_type, indigo_loader_type))
    return loader
...
def render_indigorenderer(app, text, options):
    ...
    try:
        ...
        # Set defaul options
        indigo.setOption('render-bond-length', '37.5')
        ...
        indigoObject = switchIndigoLoader(indigo, options)(text)
        indigoRenderer.renderToFile(indigoObject, absolute_path)
